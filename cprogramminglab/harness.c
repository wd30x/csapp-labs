...
{
    bool result = called_malloc;
    called_malloc = false;
    return result;
}

static void *allocate_memory(size_t size, bool should_zero_memory)
{
    called_malloc = true;
    if (noallocate_mode)
    {
        report_event(MSG_FATAL, "Calls to malloc or calloc disallowed");
        return NULL;
    }
    if (fail_allocation())
    {
        report_event(MSG_WARN, "Malloc or calloc returning NULL");
        return NULL;
    }
...
{
    return allocate_memory(num * size, true);
}
